...
export function emailSignIn(user) {
  ...
  return signInWithEmailAndPassword(newAuth, user.email, user.pw);
}

export function emailRegistration(user) {
  const newAuth = getAuth();
  return createUserWithEmailAndPassword(newAuth, user.email, user.pw);
}
